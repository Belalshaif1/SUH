/**
 * @file server/controllers/fees.controller.js
 * @description Business logic for fees CRUD operations.
 */
    ...
    try {
        ...
        const formatted = feesList.map(f => ({
            ...
            departments: f.department_name_ar ? {
                ...
                colleges: f.college_name_ar ? { name_ar: f.college_name_ar, name_en: f.college_name_en } : null,
            } : null,
        }));
...
async function update(req, res) {
    try {
        ...
        const target = await db.getAsync(
            'SELECT f.id, c.university_id, d.college_id, f.department_id FROM fees f JOIN departments d ON f.department_id = d.id JOIN colleges c ON d.college_id = c.id WHERE f.id = $1',
            [req.params.id]
        );
...
async function remove(req, res) {
    try {
        const target = await db.getAsync(
            'SELECT f.id, c.university_id, d.college_id FROM fees f JOIN departments d ON f.department_id = d.id JOIN colleges c ON d.college_id = c.id WHERE f.id = $1',
            [req.params.id]
        );
        ...
        if (req.user.role !== 'super_admin') {
            if (req.user.university_id && target.university_id !== req.user.university_id) return res.status(403).json({ error: 'Access denied' });
            if (req.user.role === 'college_admin' && req.user.college_id !== target.college_id) return res.status(403).json({ error: 'Access denied' });
        }
