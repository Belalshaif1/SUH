/**
 * @file server/controllers/sync.controller.js
 * @description Business logic for offline sync operations (pull/push).
 */

const db = require('../config/db');

const SYNC_TABLES = ['universities', 'colleges', 'departments', 'announcements', 'research', 'graduates', 'jobs'];
...
async function pull(req, res) {
    ...
    try {
        ...
        for (const table of SYNC_TABLES) {
            ...
            updates.forEach(row => allUpdates.push({ table, data: row }));
        }
    ...
    try {
        for (const mutation of mutations) {
            ...
            if (action === 'create' || action === 'update') {
                ...
                await db.query(
                    `INSERT INTO ${table} (${keys.join(', ')}) VALUES (${placeholders}) ON CONFLICT (id) DO UPDATE SET ${updates}, updated_at = CURRENT_TIMESTAMP`,
                    values
                );
