/**
 * @file server/controllers/graduates.controller.js
 * @description Business logic for graduates CRUD operations.
 */
...
async function getAll(req, res) {
    try {
        ...
        if (req.query.department_id) { query += ` AND g.department_id = $${paramIdx++}`; params.push(req.query.department_id); }
...
async function update(req, res) {
    try {
        ...
        const target = await db.getAsync(
            'SELECT g.id, c.university_id, d.college_id, g.department_id FROM graduates g JOIN departments d ON g.department_id = d.id JOIN colleges c ON d.college_id = c.id WHERE g.id = $1',
            [req.params.id]
        );
