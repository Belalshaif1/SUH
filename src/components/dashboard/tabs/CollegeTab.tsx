...
export const CollegeTab: React.FC<CollegeTabProps> = ({
  ...
  // 1. تصفية الكليات بناءً على دور المستخدم (RBAC) لضمان عدم رؤية بيانات غير مخولة
  const filteredColleges = colleges.filter(c => {
    if (role === 'super_admin') return true; // المدير العام يرى الكل
    if (role === 'university_admin') return c.university_id === userRole?.university_id; // مدير الجامعة يرى كلياته فقط
    if (role === 'college_admin') return c.id === userRole?.college_id; // مدير الكلية يرى كليته فقط
    return false;
  });

  // 2. معالجة القائمة (ترتيب الأبجدي + تثبيت العناصر المثبتة في الأعلى)
  const processedList = processData(filteredColleges);

  return (
    <div className="p-6">
      {/* 3. الجزء العلوي: العنوان وزر الإضافة */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
        ...
        {/* زر الإضافة: يظهر فقط إذا كان المستخدم يملك صلاحية الإضافة */}
        {canAdd && (
      ...
      {/* 4. شبكة عرض الكليات (Grid Layout) */}
          ...
          <Card key={c.id} className="card-premium group hover:scale-[1.02] transition-all duration-300 border-none shadow-lg">
            <CardContent className="flex items-center justify-between p-6">
              <div className="flex items-center gap-4">
                {/* أيقونة الحالة: ذهبية إذا كانت مثبتة، وعادية خلاف ذلك */}
                {c.is_pinned ? (
