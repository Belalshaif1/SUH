...
const AdminManagement: React.FC<AdminManagementProps> = ({ universities, colleges, departments }) => {
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            toast({ title: err.message || (isAr ? 'حدث خطأ أثناء العملية' : 'An error occurred'), variant: 'destructive' });
        } finally {
    ...
    const renderRoleFields = (fs: any, set: (v: any) => void, fc: any[], fd: any[]) => (
        <>
            ...
            {/* University selector — ONLY for super_admin to pick the target university */}
            {fs.role && fs.role !== 'super_admin' && role === 'super_admin' && (
                <div className="space-y-1">
                        ...
                        <SelectContent>
                            {universities.map(u => (
                                <SelectItem key={u.id} value={u.id}>
                                    {isAr ? u.name_ar : (u.name_en || u.name_ar)}
                                </SelectItem>
                            ))}
                        </SelectContent>
                    </Select>
            ...
            {['college_admin', 'department_admin'].includes(fs.role) && (role === 'super_admin' || role === 'university_admin') && (
                <div className="space-y-1">
            ...
            {fs.role === 'department_admin' && (role === 'super_admin' || role === 'university_admin' || role === 'college_admin') && (
                <div className="space-y-1">
