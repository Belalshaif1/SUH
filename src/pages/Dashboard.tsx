          ...
          <Tabs defaultValue={role === 'super_admin' ? 'users' : 'admins'} className="w-full">
            ...
            <div className="px-6 pt-6 border-b border-primary/5 bg-slate-50/30">
              <TabsList className="flex flex-wrap gap-2 h-auto mb-4 bg-transparent p-0">
                ...
                {hasPermission('manage_jobs') && (role === 'college_admin' || role === 'super_admin') && <TabsTrigger value="jobs" className="tab-trigger-premium"><Briefcase className="h-4 w-4" />{t('nav.jobs')}</TabsTrigger>}
            ...
            <TabsContent value="universities"><UniversityTab universities={data.universities} onAdd={() => dialogs.openAdd('university')} onEdit={(i) => dialogs.openEdit('university', i)} onDelete={(id) => actions.handleDelete('universities', id)} processData={data.processData} /></TabsContent>
            <TabsContent value="colleges"><CollegeTab colleges={data.colleges} onAdd={() => dialogs.openAdd('college')} onEdit={(i) => dialogs.openEdit('college', i)} onDelete={(id) => actions.handleDelete('colleges', id)} processData={data.processData} /></TabsContent>
            <TabsContent value="departments"><DepartmentTab departments={data.departments} onAdd={() => dialogs.openAdd('department')} onEdit={(i) => dialogs.openEdit('department', i)} onDelete={(id) => actions.handleDelete('departments', id)} processData={data.processData} /></TabsContent>
            <TabsContent value="announcements"><AnnouncementsTab announcements={data.announcements} onAdd={() => dialogs.openAdd('announcement')} onEdit={(i) => dialogs.openEdit('announcement', i)} onDelete={(id) => actions.handleDelete('announcements', id)} processData={data.processData} /></TabsContent>
